...
- **rules/analyzers** apply the heuristics shipped in `quotecraft/core.py`.
- **score** ranks by severity.
- **MCP server** (`quotecraft mcp`) exposes `scan` for Cognis.Studio agents.
