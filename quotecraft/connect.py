"""Native cognis-connect emit for quotecraft — forward findings to any platform.

Maps quotecraft's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    quotecraft ... --format json | quotecraft-emit --to stix
    quotecraft-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['severity'] = rec.get('confidence', 'low')
